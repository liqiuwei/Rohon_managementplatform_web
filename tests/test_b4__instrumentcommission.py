...
class TestSettingFundst(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.accountopening = IndexPage(cls.driver).operating_account_management2().input_account(
            ...
            click_operation(cls.data).click_configuration(cls.data).instrumentcommission()
    ...
    def test_instrumentcommission_a1_succeed(self):
        """期货合约手续费率配置-绝对收取得用例"""
        data = self.accountopening.variety_earnest_newly().import_contract().shoushu_shouxufei().jine_shouxufei() \
            .variety_configuration_queding().get_Hand_more2()
        self.accountopening.click_quanxuan().plscaccountcommissioncode().window_affirm2()
        try:
    ...
    def test_instrumentcommission_a2_succeed(self):
        """期货合约手续费率配置-绝对收取得用例"""
        data = self.accountopening.variety_earnest_newly().ChargeType().import_contract().shoushu_shouxufei().jine_shouxufei() \
            .variety_configuration_queding().get_Hand_more2()
        self.accountopening.click_quanxuan().plscaccountcommissioncode().window_affirm2()
        try:
            self.assertTrue(data['按手数开仓'] == '100.00 元')
    ...
    def test_instrumentcommission_a3_succeed(self):
        """期货合约手续费率配置-绝对收取得用例"""
        data = self.accountopening.variety_earnest_newly().ChargeType3().import_contract().shoushu_shouxufei().jine_shouxufei() \
            .variety_configuration_queding().get_Hand_more2()
        self.accountopening.click_quanxuan().plscaccountcommissioncode().window_affirm2()
        try:
